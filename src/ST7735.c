...
void CleanRect(int16_t x, int16_t y, int16_t w, int16_t h) {
  ...
  if(y >= _height - 1){                     // image exceeds bottom of screen
    h = h - (y - _height + 1);
    y = _height - 1;
  }
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){	// 0x07E0 green to be removed
				int16_t drawX = oriX + x;
				int16_t drawY = oriY - (h - y) + 1;
				int16_t xPut = (drawX - 8)/16;
				int16_t yPut = 0;
				if(drawY < 16) yPut = 0;
				else if(drawY < 32) yPut = 1;
				else if(drawY < 48) yPut = 2;
				else if(drawY < 64) yPut = 3;
				else if(drawY < 80) yPut = 4;
				else if(drawY < 96) yPut = 5;
				else if(drawY < 112) yPut = 6;
				else if(drawY < 128) yPut = 7;
				else if(drawY < 144) yPut = 8;
				else if(drawY < 160) yPut = 9;
				if(yPut < 0) yPut = 0;
				const unsigned short * reDrawnTile = getMapTile(xPut, yPut);
				int16_t xTile = (xPut*16) + 8;
				int16_t yTile = (yPut*16) + 15;
				int16_t pixelIndex = drawX - xTile;
				pixelIndex = pixelIndex + (yTile - drawY)*16;
				uint16_t newPixel = reDrawnTile[pixelIndex];
...
void SpriteDraw (int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
	int16_t oriX = x;
	int16_t oriY = y;
  int16_t skipC = 0;                      // non-zero if columns need to be skipped due to clipping
  int16_t originalWidth = w;              // save this value; even if not all columns fit on the screen, the image is still this width in ROM
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){	// 0x07E0 green to be removed                             // send the top 8 bits
			if(image[i] != 0xAEBF){
				writedata((uint8_t)(image[i] >> 8));
																					// send the bottom 8 bits
				writedata((uint8_t)image[i]);
