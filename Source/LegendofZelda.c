...
0, 3, 1, South, 0, 0, 0, 0, 10, 4, 0, 0, 0 //Link example
}};
...
void update_Hearts(void){
		updateHeart = 0;
				...
				for(HeartIndex = 0; HeartIndex < Characters[0].life/2; HeartIndex ++){
					ST7735_DrawBitmap(1, leftoffset, Heart, 6, 5);
					leftoffset -= 6;
				}
				for(HeartIndex = 0; HeartIndex < 5 - (Characters[0].life/2); HeartIndex ++){
					ST7735_DrawBitmap(1, rightoffset, EmptyHeart, 6, 5);
					rightoffset += 6;
				}
				if(Characters[0].life % 2 != 0){
					ST7735_DrawBitmap(1, leftoffset, HalfHeart, 6, 5);
					leftoffset -= 6;
				}
}
...
void use_Item(void){
		if(activeItem == 0){
			if(!(bombCountingDown) && canPlaceItem(Characters[0].direction) && UseItem[0].amount > 0){
				...
				drawItem(UseItem[0].type, UseItem[0].xpos, UseItem[0].ypos);
			}useItem = 0;
		}
		else{ 
...
void Play_Game (void){
		if(!selectScreen){
			...
			if(mapPt != 17){
				for(itemPt = 0; itemPt < 20; itemPt++){
					if(ActiveItems[itemPt].active && ActiveItems[itemPt].display){
						setGridPosition(ActiveItems[itemPt].label, ActiveItems[itemPt].xpos, ActiveItems[itemPt].ypos);
						drawItem(ActiveItems[itemPt].type, ActiveItems[itemPt].xpos, ActiveItems[itemPt].ypos);
					}
				}
			...
			// Rewrite this to get rid of bug
			...
			// Updates sprite position by moving it
